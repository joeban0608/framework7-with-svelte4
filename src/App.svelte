<script>
	...
	const customButtons = {
		...
		h4: {
			content: 'H4',
			onClick(editor, buttonEl) {
				document.execCommand('formatBlock', false, 'h4'); // 插入 H4
			}
		},
		h5: {
			content: 'H5',
			onClick(editor, buttonEl) {
				document.execCommand('formatBlock', false, 'h5'); // 插入 H5
			}
		},
		h6: {
			content: 'H6',
			onClick(editor, buttonEl) {
				document.execCommand('formatBlock', false, 'h6'); // 插入 H6
			}
		}
	};
...
<App {...f7Params}>
	<Page>
		...
		<TextEditor
			placeholder="Enter text..."
			{customButtons}
			buttons={[['bold', 'italic', 'underline', 'strikeThrough', 'hr', 'h4', 'h5', 'h6']]}
		/>
